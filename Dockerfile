...
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w" -o /out/aiclient-kiro ./cmd/server

FROM alpine:3.22

RUN apk add --no-cache ca-certificates wget && adduser -D -h /app app
WORKDIR /app
COPY --from=go-builder /out/aiclient-kiro /app/aiclient-kiro
COPY config /app/defaults
RUN mkdir -p /app/config && chown -R app:app /app
USER app

EXPOSE 13120 19876-19880
VOLUME ["/app/config"]

HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD wget -qO- http://127.0.0.1:13120/health >/dev/null || exit 1

CMD ["/app/aiclient-kiro", "--config", "/app/config/config.toml"]
